...
public class UnitTest {
    ...
    @Test
    void orderConstructorUsesProvidedId() {
        Order order = new Order(42, null, null, new Date());
        assertTrue(order.toString().contains("Pedido #42"));
    }

    @Test
    void employeeAltConstructorShouldNotThrow() {
        assertDoesNotThrow(() -> newEmployeeViaAltCtor(1, "Luis", "clave123", "vendedor"));
    }

    @Test
    void employeeValidatePasswordMatchesStoredValue() {
        Employee employee = new Employee(1, "Name", "Role", "user", "secret");
        boolean ok = assertDoesNotThrow(() -> employee.validatePassword("secret"));
        boolean bad = assertDoesNotThrow(() -> employee.validatePassword("wrong"));
        assertTrue(ok);
        assertFalse(bad);
    }

    @Test
    void stockFormControllerCalculatesGainUsingPercent() {
        ...
        assertEquals("12.00", view.getFinalPriceText());
        assertEquals("2.00", view.getGainValueText());
    }

    @Test
    void orderDetailRejectsNegativeQuantity() {
        Product product = new Product("P1", "Dog Food", 10.0, 5, "FOOD", "DOG", "M", "Brand");
        assertThrows(IllegalArgumentException.class, () -> new OrderDetail(product, -2));
    }

    @Test
    void productValidatorRejectsNegativeCost() {
        ...
        assertTrue(errors.containsKey("txtCostProduct"));
    }
