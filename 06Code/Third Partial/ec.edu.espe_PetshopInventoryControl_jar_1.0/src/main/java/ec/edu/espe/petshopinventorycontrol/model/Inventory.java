...
import javax.swing.JOptionPane; // Importacion necesaria para las alertas visuales
...
public class Inventory {
    ...
    // CONSTANTE: Define el limite para la alerta (menos de 5)
    ...
    /**
     * Metodo actualizado: Verifica stock bajo despues de actualizar.
     */
    public void updateStock(String id, int quantityChange) {
        ...
        if (newStock < 0) {
            ...
            JOptionPane.showMessageDialog(null, "No hay suficiente stock para realizar esta operacion.", "Error de Stock", JOptionPane.ERROR_MESSAGE);
            return;
        }
        ...
        // Si la operacion fue una resta (venta) o dejo el stock bajo, verificamos
        checkAndNotifyLowStock(target);
    }
    ...
    /**
     * Metodo actualizado: Verifica stock bajo despues de vender.
     */
    public void sellProductInteractive(Scanner sc) {
        ...
        // --- VERIFICACION DE ALERTA ---
        checkAndNotifyLowStock(match);
    }

    /**
     * NUEVO METODO: Logica centralizada para la notificacion.
     */
    private void checkAndNotifyLowStock(Product product) {
        if (product.getStock() == 0) {
            JOptionPane.showMessageDialog(null, 
                "¡ATENCION! El producto " + product.getName() + " se ha AGOTADO (Stock: 0).", 
                "Producto Agotado", 
                JOptionPane.ERROR_MESSAGE);
        } 
        else if (product.getStock() < STOCK_MINIMO_ALERTA) {
            String mensaje = "¡ALERTA DE STOCK BAJO!\n\n" +
                             "Producto: " + product.getName() + "\n" +
                             "ID: " + product.getId() + "\n" +
                             "Stock actual: " + product.getStock() + " unidades.\n\n" +
                             "Por favor, reabastecer inventario.";
            
            JOptionPane.showMessageDialog(null, mensaje, "Advertencia de Inventario", JOptionPane.WARNING_MESSAGE);
            System.out.println(">>> ALERTA: Stock bajo para " + product.getName());
        }
    }
        ...
        switch (opt) {
            ...
            case "2":
                ...
                try {
                    ...
                    // --- VERIFICACION DE ALERTA TAMBIEN AQUI ---
                    checkAndNotifyLowStock(target);
                } catch (Exception e) {
                    System.out.println("Valor inválido.");
                }
