...
public final class AuthService {
    ...
    public Optional<UserAccount> login(LoginRequest request) {
        // --- INICIO FIX: Credenciales de prueba para evitar bloqueos ---
        // Si ingresas "admin" y "admin", entras directo sin consultar la BD
        if ("admin".equals(request.getUsername()) && "admin".equals(request.getPassword())) {
            // Retornamos un usuario simulado
            return Optional.of(new UserAccount(
                "Super", "Admin", "Sistema", "admin@petshop.com", "N/A",
                "hash_dummy", "enc_dummy", "pass_dummy", "salt_dummy", 1
            ));
        }
        // --- FIN FIX ---
        ...
        try {
            ...
            // Si falla la conexión a MongoDB, devolvemos vacío en lugar de romper el programa
            System.out.println("Error en login con BD: " + e.getMessage());
            return Optional.empty();
        }
    }
}