...
public class PetShopMain {
    ...
    private static final Gson GSON = new Gson(); // Usamos GSON para convertir Product a Documento
        ...
        while (true) {
            ...
            switch (opt) {
                case "1":
                    ...
                    // inventory.saveToJson(PRODUCTS_JSON); // REMOVIDO: El guardado se hace DENTRO de addProductDetailed a MongoDB
                    break;
                case "2":
    ...
    private static void addProductDetailed(Inventory inventory) {
        while (true) {
            try {
                ...
                // 1. Crea el objeto Producto (igual que antes)
                Product product = new Product(id, name, price, stock, category, animal, size, brand);
                inventory.addProduct(product);

                // =========================================================================
                // *** CÓDIGO PARA GUARDAR EN MONGODB ATLAS ***

                // 2. Convierte el objeto de Java (Product) a un texto JSON
                String jsonProduct = GSON.toJson(product);
                
                // 3. Convierte el texto JSON a un Documento (el formato que MongoDB usa)
